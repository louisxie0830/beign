    ...
    <div class="main-content">
      ...
      >
        ...
        <template slot="username" slot-scope="text, record">
          <template>
            <span>{{ text }}</span>
            <div v-if="loginUserInfo.id === record.id">
        ...
        <template slot="Operation" slot-scope="text, record">
          <span>
            <a-button type="primary" size="small" @click="() => editUserInfo(record.id, 'm-buttonnage')">{{
              $t('superadmin.edit.user.data')
            }}</a-button>
            <a-button size="small" @click="() => inviteUserRole(record.id, 'manage')" style="margin-top: 5px;">{{
      ...
      <div align="right" style="margin-bottom: 40px;">
        <!-- <a-button type="primary" @click="triggerInviteModal">Invite New Entity Role</a-button> -->
...
import editUser from '../superAdmin/components/EditUser'
import createUser from '../superAdmin/components/CreateUser'
import inviteUserRole from '../superAdmin/components/InviteUserRole'
...
export default {
  ...
  methods: {
    handleSelect () {},
    initData () {
      const companyId = this.loginUserInfo.companyId
      this.$store
    ...
    SAVE () {
      if (this.selectedRowKeys.length === 0) {
        return this.$message.warning(this.$t('superadmin.please.select.user.permission'))
      }
