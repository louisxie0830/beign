...
var optionsSchema = Joi.object({
  locales: Joi.array()
    .items(Joi.string())
    .default(defaultOptions.locales),
  default: Joi.string()
    .allow(null)
    .default(defaultOptions.default),
  configFile: Joi.string()
    .allow(null)
    .default(defaultOptions.configFile),
  configKey: Joi.string()
    .allow(null)
    .default(defaultOptions.configKey),
  param: Joi.string()
    .allow(null)
    .default(defaultOptions.param),
  query: Joi.string()
    .allow(null)
    .default(defaultOptions.query),
  cookie: Joi.string()
    .allow(null)
    .default(defaultOptions.cookie),
  cookieKey: Joi.string()
    .allow(null)
    .default(defaultOptions.cookieKey),
  header: Joi.string()
    .allow(null)
    .default(defaultOptions.header),
  order: Joi.array()
  ...
  getter: Joi.string()
    .allow(null)
    .default(defaultOptions.getter),
  setter: Joi.string()
    .allow(null)
    .default(defaultOptions.setter),
  attribute: Joi.string()
    .allow(null)
    .default(defaultOptions.attribute),
...
Internal.prototype.getAvailableLocales = function getAvailableLocales() {
  ...
  if (locales.length === 0 && this.options.configFile) {
    ...
    if (!fileExists(this.options.configFile, false)) {
      throw new Error(
        'Configuration file "' + this.options.configFile + '" cannot be found'
      );
    }

    locales = lodash.get(
      require(this.options.configFile),
      this.options.configKey
    ); // key chain string to reference: 'options.locale' => options.locale
    if (!Array.isArray(locales)) locales = [];
  }
...
Internal.prototype.processRequest = function processRequest(request, h) {
  ...
  // console.log(locale)
  if (locale === null) {
    const l = request.headers["x-being-area"];
    if (l === "cn") {
      locale = "zh_cn";
    } else if (l === "tw") {
      locale = "zh_tw";
...
exports.plugin = {
  ...
  register: async function(server, options) {
     ...
     */
    server.expose("getDefaultLocale", function getDefaultLocale(request) {
      const l = request.header["x-being-area"];
      if (l === "cn") {
        return "zh-cn";
      }
      if (l === "tw") {
    ...
    server.expose("getLocale", function getLocale(request) {
      try {
        const requestLang = request.header["accept-language"];
        const actLang = lodash.get(request, internal.options.getter)();
        console.log(requestLang, actLang);
        if (requestLang === actLang) {
          return actLang;
        } else {
          const l = request.header["x-being-area"];
          if (l === "cn") {
