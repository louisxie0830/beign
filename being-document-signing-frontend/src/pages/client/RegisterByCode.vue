          ...
          <v-card>
            <v-card-text>
              <v-form ref="form"
                ...
                <v-flex xs12>
                  <p class="b-label"
                     v-text="$t('invite_friends.company.name')" />
                </v-flex>
                <v-text-field outline
                              required
                              :disabled="true"
                              class="b-input"
                              v-model.trim="param.company.name"
                              validate-on-blur />
                <v-flex xs12>
                  <p class="b-label"
                ...
                <v-text-field outline
                              ...
                              class="b-input"
                              :disabled="true"
                              v-model.trim="param.department.name"
                              validate-on-blur/>
                <v-flex xs12>
                  <p class="b-label"
...
export default {
    ...
    next(vm => {
      if (from.name === 'Policy' || from.name === 'Privacy') {
        ...
        vm.param = {
          ...
          company: getCacheRegData.company || { id: null, name: null },
          department: getCacheRegData.department || { id: null, name: null }
        };
      }
    });
  },
  beforeRouteLeave(to, from, next) {
    if (to.name === 'Policy' || to.name === 'Privacy') {
      ...
      const cacheRegData = {
        ...
        company: company || { id: null, name: null },
        department: department || { id: null, name: null }
      };
  ...
  created() {
    ...
    this.GET_INVITED_INFO(this.$route.query.inviteCode).then(({ data }) => {
      this.param.name = data.data.name;
      this.param.email = data.data.email;
      this.param.phone = data.data.mobile;
      this.param.company = data.data.company;
      this.param.department = data.data.department;
      this.param.inviteCode = this.$route.query.inviteCode;
      this.param.countryCodes.forEach(item => {
        if (item.iso2 === data.data.country) this.param.code = item.code;
      });
    });
  },
  methods: {
    ...mapActions({
      ...
      GET_INVITED_INFO: actions.GET_INVITED_INFO
    }),
    ...mapMutations({
