<template>
  <canvas v-visible.once="drawPage"
          @click="clickPage"
          v-bind="canvasAttrs"
          class="spec-canvas"></canvas>
</template>
...
export default {
  ...
  computed: {
    ...
    getMaxPage() {
      return this.page.transport.numPages;
    },
    canvasStyle() {
    ...
    drawPage() {
      ...
      this.renderTask
        .then(data => {
          ...
          if (this.getMaxPage === this.pageNumber) {
            this.$emit('page-rendered-completed', {
              page: this.page,
              text: `Rendered completed page ${this.pageNumber}`
            });
          }
        })
        .catch(response => {
